from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path

import cv2
import numpy as np


ROOT = Path(r"d:\DATAGUESS\datamatrix_v2")
OUT_DIR = ROOT / "artifacts" / "cropped_steps" / "step05_hessian_blobness"
OUT_DIR.mkdir(parents=True, exist_ok=True)


@dataclass
class MapResult:
    name: str
    count: int
    matched: int
    precision: float
    recall: float
    f1: float
    points: np.ndarray


def extract_red_centers(marked_bgr: np.ndarray) -> np.ndarray:
    b = marked_bgr[:, :, 0].astype(np.int16)
    g = marked_bgr[:, :, 1].astype(np.int16)
    r = marked_bgr[:, :, 2].astype(np.int16)
    mask = (r > 160) & (r - g > 60) & (r - b > 60)
    num, _, stats, centroids = cv2.connectedComponentsWithStats(mask.astype(np.uint8), 8)
    pts: list[tuple[float, float]] = []
    for idx in range(1, num):
        area = int(stats[idx, cv2.CC_STAT_AREA])
        if 3 <= area <= 400:
            x, y = centroids[idx]
            pts.append((float(x), float(y)))
    return np.array(pts, dtype=np.float32)


def local_contrast_normalize(gray: np.ndarray, win: int = 41, eps: float = 1e-3) -> np.ndarray:
    gray_f = gray.astype(np.float32) / 255.0
    mu = cv2.GaussianBlur(gray_f, (0, 0), win / 6.0)
    sq_mu = cv2.GaussianBlur(gray_f * gray_f, (0, 0), win / 6.0)
    sigma = np.sqrt(np.maximum(sq_mu - mu * mu, 0.0))
    lcn = (gray_f - mu) / (sigma + eps)
    return lcn


def hessian_blobness(image: np.ndarray, sigma: float) -> tuple[np.ndarray, np.ndarray]:
    smooth = cv2.GaussianBlur(image, (0, 0), sigmaX=sigma, sigmaY=sigma)
    dxx = cv2.Sobel(smooth, cv2.CV_32F, 2, 0, ksize=3)
    dyy = cv2.Sobel(smooth, cv2.CV_32F, 0, 2, ksize=3)
    dxy = cv2.Sobel(smooth, cv2.CV_32F, 1, 1, ksize=3)
    det = dxx * dyy - dxy * dxy
    trace = dxx + dyy
    disc = np.sqrt(np.maximum(trace * trace - 4.0 * det, 0.0))
    l1 = 0.5 * (trace + disc)
    l2 = 0.5 * (trace - disc)
    same_sign = (l1 > 0) & (l2 > 0)
    isotropy = np.minimum(np.abs(l1), np.abs(l2)) / (np.maximum(np.abs(l1), np.abs(l2)) + 1e-6)
    blobness = np.where(same_sign, np.sqrt(np.maximum(det, 0.0)) * isotropy, 0.0)
    return det, blobness


def normalize_u8(image: np.ndarray) -> np.ndarray:
    return cv2.normalize(image, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)


def find_peaks(map_u8: np.ndarray, quantile: float, min_dist: float = 8.0) -> np.ndarray:
    f = map_u8.astype(np.float32) / 255.0
    dil = cv2.dilate(f, np.ones((5, 5), np.uint8))
    threshold = float(np.quantile(f, quantile))
    mask = (f >= dil - 1e-7) & (f >= threshold)
    ys, xs = np.where(mask)
    scores = f[ys, xs]
    order = np.argsort(scores)[::-1]
    pts: list[tuple[float, float]] = []
    r2 = min_dist * min_dist
    for idx in order:
        x = float(xs[idx])
        y = float(ys[idx])
        if all((x - px) ** 2 + (y - py) ** 2 > r2 for px, py in pts):
            pts.append((x, y))
    return np.array(pts, dtype=np.float32)


def match_metrics(pred: np.ndarray, truth: np.ndarray, tol: float = 6.0) -> tuple[int, float, float, float]:
    if len(pred) == 0 or len(truth) == 0:
        return 0, 0.0, 0.0, 0.0
    d = np.linalg.norm(pred[:, None, :] - truth[None, :, :], axis=2)
    used: set[int] = set()
    matched = 0
    for i in range(d.shape[0]):
        j = int(np.argmin(d[i]))
        if d[i, j] <= tol and j not in used:
            used.add(j)
            matched += 1
    precision = matched / max(1, len(pred))
    recall = matched / max(1, len(truth))
    f1 = 0.0 if precision + recall == 0 else (2 * precision * recall / (precision + recall))
    return matched, precision, recall, f1


def draw_overlay(raw: np.ndarray, truth: np.ndarray, pred: np.ndarray, path: Path) -> None:
    vis = raw.copy()
    for x, y in truth:
        cv2.circle(vis, (int(round(x)), int(round(y))), 4, (0, 0, 255), 1, lineType=cv2.LINE_AA)
    for x, y in pred:
        cv2.circle(vis, (int(round(x)), int(round(y))), 4, (0, 255, 0), 1, lineType=cv2.LINE_AA)
    cv2.imwrite(str(path), vis)


def evaluate(name: str, raw: np.ndarray, truth: np.ndarray, map_u8: np.ndarray, quantile: float) -> MapResult:
    pts = find_peaks(map_u8, quantile=quantile, min_dist=8.0)
    matched, precision, recall, f1 = match_metrics(pts, truth)
    draw_overlay(raw, truth, pts, OUT_DIR / f"{name}_overlay.png")
    return MapResult(name, int(len(pts)), matched, precision, recall, f1, pts)


def main() -> None:
    raw = cv2.imread(str(ROOT / "cropped.png"))
    marked = cv2.imread(str(ROOT / "cropped_isaretli.png"))
    if raw is None or marked is None:
        raise SystemExit("Missing input image.")
    gray = cv2.cvtColor(raw, cv2.COLOR_BGR2GRAY)
    truth = extract_red_centers(marked)

    lcn = local_contrast_normalize(gray, win=41)
    dark_lcn = np.clip(-lcn, 0.0, None)
    dark_lcn_u8 = normalize_u8(dark_lcn)
    cv2.imwrite(str(OUT_DIR / "lcn_dark.png"), dark_lcn_u8)

    results: list[MapResult] = []
    blob_maps: dict[str, np.ndarray] = {}
    for sigma in (1.4, 1.8, 2.2, 2.8):
        det, blob = hessian_blobness(dark_lcn, sigma=sigma)
        det_pos = np.clip(det, 0.0, None)
        det_u8 = normalize_u8(det_pos)
        blob_u8 = normalize_u8(blob)
        blob_maps[f"det_sigma_{sigma:.1f}"] = det_u8
        blob_maps[f"blob_sigma_{sigma:.1f}"] = blob_u8
        cv2.imwrite(str(OUT_DIR / f"det_sigma_{sigma:.1f}.png"), det_u8)
        cv2.imwrite(str(OUT_DIR / f"blob_sigma_{sigma:.1f}.png"), blob_u8)

    det_stack = np.stack([blob_maps[k] for k in blob_maps if k.startswith("det_")], axis=0).astype(np.float32)
    blob_stack = np.stack([blob_maps[k] for k in blob_maps if k.startswith("blob_")], axis=0).astype(np.float32)
    det_multi = normalize_u8(det_stack.max(axis=0))
    blob_multi = normalize_u8(blob_stack.max(axis=0))
    cv2.imwrite(str(OUT_DIR / "det_multi.png"), det_multi)
    cv2.imwrite(str(OUT_DIR / "blob_multi.png"), blob_multi)

    eval_specs = [
        ("lcn_dark", dark_lcn_u8, 0.955),
        ("det_multi", det_multi, 0.965),
        ("blob_multi", blob_multi, 0.955),
        ("blob_sigma_1.4", blob_maps["blob_sigma_1.4"], 0.955),
        ("blob_sigma_1.8", blob_maps["blob_sigma_1.8"], 0.955),
        ("blob_sigma_2.2", blob_maps["blob_sigma_2.2"], 0.955),
        ("blob_sigma_2.8", blob_maps["blob_sigma_2.8"], 0.955),
    ]

    for name, image, quantile in eval_specs:
        results.append(evaluate(name, raw, truth, image, quantile))
    results.sort(key=lambda x: x.f1, reverse=True)

    lines = ["truth_count=%d" % len(truth)]
    for res in results:
        lines.append(
            f"{res.name}: count={res.count} matched={res.matched} "
            f"precision={res.precision:.3f} recall={res.recall:.3f} f1={res.f1:.3f}"
        )
    (OUT_DIR / "summary.txt").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))


if __name__ == "__main__":
    main()
